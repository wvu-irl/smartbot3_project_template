# move_to_waypoint.py, here robot rotates first to align the heading and moves in a straight line
import math
...
class Params:
    ...
    phase: str = 'rotate'  # 'rotate' -> 'translate' -> 'done'
...
def step(bot: SmartBotType, params: Params, states: State) -> None:
    ...
    if params.phase == 'rotate':
        # Step 1: spin in place until we're facing the waypoint.
        if abs(heading_error) > params.heading_tol:
            ang_vel = params.rotational_gain * heading_error
        else:
            params.phase = 'translate'
            print('heading aligned -> translating to waypoint')

    elif params.phase == 'translate':
        # Step 2: drive straight at the waypoint (no more turning) using P control on distance.
    ...
    print(f'phase={params.phase} distance={distance:.3f} heading_error={heading_error:.3f}')
