...
def ant_controller(
    ...
    if sensors.seen_hexes and sensors.seen_hexes.poses:
        ...
        goal_ang = atan2(y_left, x_forward)
        dist_to_goal = math.hypot(x_forward, y_left)
    ...
    # front sector: ±30 degrees (adjust as needed)
    front_width = np.deg2rad(120)  # total width
    half = front_width / 2
    ...
    for i in range(n):
        ...
        if ang < -half or ang > half:
            # if ranges[i] < 0.1:  # your original threshold
            ranges[i] = np.nan
